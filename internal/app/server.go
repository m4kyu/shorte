...
func (s *Server) handleHome(w http.ResponseWriter, r *http.Request) {
	...
	_ = s.tpl.Execute(w, map[string]any{"UserID": uid})
}
...
<body>
  <h1>shorte</h1>
  <p>Current user id: <b id="uid">{{.UserID}}</b></p>
  <div class="grid">
    <div class="card">
      <div class="row">
        <h3>Auth</h3>
  ...
  <script>
    ...
    async function call(path, method, body, quiet=false) {
      ...
      if (!quiet) out.textContent = t;
      let json = null;
      try { json = JSON.parse(t); } catch (_e) {}
      return {status: r.status, json, text: t};
    }
    async function register(){ await call('/api/v1/auth/register','POST',{email:val('email'),password:val('password')}); await loadLinks(); }
    async function login(){ await call('/api/v1/auth/login','POST',{email:val('email'),password:val('password')}); await loadLinks(); }
    async function logout(){ await call('/api/v1/auth/logout','POST'); await loadLinks(); }
